...
// The mock body lives as a string template rather than a serialized
// function so the test toolchain doesn't try to type-check the in-page
// code (which uses `window.__TAURI_INTERNALS__` and other browser-only
// surfaces) against the test runner's Node typings.
...
const state = {
  ...
  pages: new Map([
    ["Research/alpha.md", { rel_path: "Research/alpha.md", title: "Alpha note", book: "Research", tags: ["tag-a"], modified: 1700000000 }],
    ["Research/beta.md", { rel_path: "Research/beta.md", title: "Beta note", book: "Research", tags: [], modified: 1700000100 }],
    ["Daily/2026-04-29.md", { rel_path: "Daily/2026-04-29.md", title: "2026-04-29", book: "Daily", tags: [], modified: 1714000000 }],
    ["Daily/2026-04-28.md", { rel_path: "Daily/2026-04-28.md", title: "2026-04-28", book: "Daily", tags: [], modified: 1713000000 }],
    ["Daily/2026-04-27.md", { rel_path: "Daily/2026-04-27.md", title: "2026-04-27", book: "Daily", tags: [], modified: 1712000000 }],
    ["Recipes/cake.md", { rel_path: "Recipes/cake.md", title: "Cake", book: "Recipes", tags: [], modified: 1710000000 }],
    ["loose-thought.md", { rel_path: "loose-thought.md", title: "Loose thought", book: null, tags: [], modified: 1709000000 }],
  ]),
  pageBodies: new Map(),
  settings: {
    vault_path: cfg.vaultRoot, theme: "warm", shelf_style: "leather", sidebar: "left",
    page_font: "serif", daily_book: "Daily", daily_template: null, daily_reminder_time: null,
    git_remote_url: null, git_branch: null, git_auth_kind: null,
    chat_model: "sonnet-4.6", chat_context_mode: "auto",
  },
  secrets: new Set(),
...
const handlers = {
  ...
  open_vault: ({ path }) => { state.vault = { root: path, name: path.split("/").pop() || "vault" }; return state.vault; },
  close_vault: () => { state.vault = null; },
  list_books: () => state.bookOrder.map((n) => state.books.find((b) => b.name === n)).filter(Boolean),
  create_book: ({ name }) => {
  ...
  rename_book: ({ oldName, newName }) => {
    ...
    if (b) {
      b.name = newName; b.rel_path = newName;
  ...
  delete_book: ({ name, alsoDeletePages }) => {
    const deleted = []; const moved = [];
    for (const p of [...state.pages.values()]) {
      if (p.book === name) {
        if (alsoDeletePages) { state.pages.delete(p.rel_path); deleted.push(p.rel_path); }
        else {
  ...
  set_book_order: ({ names }) => { state.bookOrder = names; emit("vault-changed", null); },
  create_page: ({ book, title }) => {
    const safe = title.replace(/[^a-zA-Z0-9 _-]/g, "");
    const rel = book ? book + "/" + safe + ".md" : safe + ".md";
    state.pages.set(rel, { rel_path: rel, title: safe, book, tags: [], modified: Date.now() / 1000 });
  ...
  list_loose_pages: () => [...state.pages.values()].filter((p) => p.book === null),
  list_pages_in_book: ({ book }) => [...state.pages.values()].filter((p) => p.book === book),
  read_page: ({ relPath }) => state.pageBodies.get(relPath) || "",
  write_page: ({ relPath, body }) => { state.pageBodies.set(relPath, body); },
  list_page_titles: () => [...state.pages.values()].map((p) => ({ rel_path: p.rel_path, title: p.title })),
  find_backlinks: () => [],
  search_pages: ({ query }) => {
    const q = query.toLowerCase();
    return [...state.pages.values()]
      .filter((p) => p.title.toLowerCase().includes(q) || (state.pageBodies.get(p.rel_path) || "").toLowerCase().includes(q))
      .map((p) => ({ rel_path: p.rel_path, title: p.title, book: p.book, snippet: p.title }));
  },
  find_related: () => [],
  ...
  open_today_daily: () => {
    const rel = "Daily/today.md"; let created = false;
    if (!state.pages.has(rel)) {
      state.pages.set(rel, { rel_path: rel, title: "today", book: "Daily", tags: [], modified: Date.now() / 1000 });
      created = true;
    }
  ...
  open_vault_from_archive: ({ destDir }) => {
    state.vault = { root: destDir, name: "imported" };
    return state.vault;
  },
  get_settings: () => ({ ...state.settings }),
  set_settings: ({ patch }) => { Object.assign(state.settings, patch); return { ...state.settings }; },
  has_secret: ({ name }) => state.secrets.has(name),
  set_secret: ({ name }) => { state.secrets.add(name); },
  clear_secret: ({ name }) => { state.secrets.delete(name); },
  git_status: () => ({ initialized: false, branch: null, remote_url: null, ahead: 0, behind: 0, dirty: [], conflicted: [] }),
  ...
  "plugin:event|listen": ({ event, handler }) => {
    ...
    if (cb) {
      ...
      if (!listeners) { listeners = new Map(); state.eventListeners.set(event, listeners); }
      listeners.set(id, cb);
    }
    return id;
  },
...
const internals = {
  ...
  unregisterCallback: (id) => { state.callbacks.delete(id); },
  convertFileSrc: (path) => "mock://" + path,
  metadata: { currentWindow: { label: "main" }, currentWebview: { label: "main" } },
};

window.__TAURI_INTERNALS__ = internals;
window.__SKEIN_MOCK__ = { state, handlers, emit };
`;
